...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'C3_FocusedLinearAttention', 'C2f_FocusedLinearAttention', 'DyHeadBlockWithFaster', 'REPEMA', 'Faster_Block', 'RepVGGBlock', 'PConv', 'C3_EMA']
...
class DyHeadBlockWithFaster(nn.Module):
    """DyHead Block with three types of attention.
    HSigmoid arguments in default act_cfg follow official code, not paper.
    https://github.com/microsoft/DynamicHead/blob/master/dyhead/dyrelu.py
    """
    ...
    def __init__(self,
        ...
        self.spatial_conv_high = C3_Faster(in_channels, in_channels)
        self.spatial_conv_mid = C3_Faster(in_channels, in_channels)
        self.spatial_conv_low = Conv(in_channels, in_channels, 3, 2)
...
class C3_EMA(C3):
    def __init__(self, c1, c2, n=1, fmapsize=None, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_EMA(c_, c_, fmapsize, shortcut, g, k=(1, 3), e=1.0) for _ in range(n)))

class C2f_EMA(C2f):
    def __init__(self, c1, c2, n=1, fmapsize=None, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_EMA(self.c, self.c, fmapsize, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
...
class REPEMA(nn.Module):
    def __init__(self, channels, c2=None, factor=32):
        super(REPEMA, self).__init__()
        self.groups = factor
        ...
        self.conv3x3 = RepBlock(channels // self.groups, channels // self.groups)
