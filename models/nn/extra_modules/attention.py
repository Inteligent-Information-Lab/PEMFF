...
class ResMDA(nn.Module):
    def __init__(self, inp, reduction=32):
        super(ResMDA, self).__init__()
        self.ResCA = Res_CoordAtt(inp, reduction=reduction)
        self.channel_att = CPCA_ChannelAttention(inp, inp // reduction)

    def forward(self, x):

        return x + self.ResCA(x) + self.channel_att(x)
